...
static void set_peripheral_battery_status(struct zmk_widget_screen *widget,
    ...
    widget->state.battery_peripheral = state.level;
    draw_middle(widget->obj, widget->cbuf2, &widget->state);
}
...
static void battery_status_peripheral_update_cb(struct battery_status_state state) {
    struct zmk_widget_screen *widget;
    SYS_SLIST_FOR_EACH_CONTAINER(&widgets, widget, node) { set_peripheral_battery_status(widget, state); }
}

static struct battery_status_state battery_status_get_peripheral_state(const zmk_event_t *eh) {
...
ZMK_DISPLAY_WIDGET_LISTENER(widget_battery_status_peripheral, struct battery_status_state,
                            battery_status_peripheral_update_cb, battery_status_get_peripheral_state);

ZMK_SUBSCRIPTION(widget_battery_status, zmk_battery_state_changed);
ZMK_SUBSCRIPTION(widget_battery_status_peripheral, zmk_peripheral_battery_state_changed);
#if IS_ENABLED(CONFIG_USB_DEVICE_STACK)
...
static struct output_status_state output_status_get_state(const zmk_event_t *_eh) {
    return (struct output_status_state){
        ...
        .peripheral_connected = active_transport ? (active_transport->api->get_status().connections != ZMK_SPLIT_TRANSPORT_CONNECTIONS_STATUS_DISCONNECTED) : false,
    };
}
...
static int peripheral_conn_status_changed_cb(const struct zmk_split_transport_central *transport,
                                             struct zmk_split_transport_status status) {
    struct output_status_state state = output_status_get_state(NULL);
    output_status_update_cb(state);
    return 0;
}
...
int zmk_widget_screen_init(struct zmk_widget_screen *widget, lv_obj_t *parent) {
    ...
    if (active_transport && active_transport->api && active_transport->api->set_status_callback) {
        active_transport->api->set_status_callback(peripheral_conn_status_changed_cb);
    }
